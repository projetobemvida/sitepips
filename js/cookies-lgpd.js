...
class CookieConsentManager {
    ...
    acceptAll() {
        ...
        this.showSettingsLink();
        this.initializeAllowedCookies();
        this.updateGoogleConsentMode(); // NOVO: Avisa o Google/Meta
    ...
    // NOVO MODO: Atualiza o Consent Mode V2 e o DataLayer do GTM
    updateGoogleConsentMode() {
        ...
        gtag('consent', 'update', {
            ...
            'ad_user_data': this.categories.marketing.enabled ? 'granted' : 'denied',
            'ad_personalization': this.categories.marketing.enabled ? 'granted' : 'denied',
            'analytics_storage': this.categories.analytics.enabled ? 'granted' : 'denied'
        });

        // Envia um evento para o GTM saber que a escolha foi feita
        window.dataLayer.push({
            'event': 'cookie_consent_update',
            'consent_status': this.categories.marketing.enabled ? 'granted' : 'denied'
        });
    }
    ...
    loadGoogleAnalytics() {
        // Deixamos o Google Tag Manager (GTM) cuidar do carregamento
        console.log('Google Analytics via GTM autorizado');
    }

    loadFacebookPixel() {
        // Deixamos o Google Tag Manager (GTM) cuidar do carregamento
        console.log('Facebook Pixel via GTM autorizado');
    }
